...
def handler(event, context):
    print('Event: ', event) #  api_gateway_endpoint?user_id=2_36&ids_nb_messages=(3,12),(4,32)
    try:
        # Variables
    ...
    if 'Item' not in response:
        ...
        response = put_item_table(mapping_table, 'user_id_raw', user_id_raw, 'user_id', max_mapped_plus, verbose=False)
        response = update_table(mapping_table, 'user_id_raw', 'max', 'user_id', max_mapped_plus)
        
        # User creation - De-Mapping Table
        response = put_item_table(demapping_table, 'user_id', str(max_mapped_plus), 'user_id_raw', user_id_raw, verbose=False)
