...
def put_item_vector_table(table, key_name, key, item_name, item, verbose=False):
    return put_item_table(table, key_name, key, item_name,  
    json.dumps(item.tolist(), separators=(',', ':'), sort_keys=True, indent=4), verbose=False)

def create_new_y_user(table, key_name, key, k, item_name, verbose=False):
    response = put_item_table(table, key_name, key, item_name,
    json.dumps(np.random.rand(k).tolist(), separators=(',', ':'), sort_keys=True, indent=4), verbose=verbose)
    return response
...
def handler(event, context):
    ...
    ### Mapped id:
    user_id = str(get_mapped(mapping_table, field_user_id_raw, user_id_raw, field_user_id))
    print(user_id)
    ...
    if 'Item' not in response:
        try:
            response = put_item_vector_table(gradient_y_table, 'user_id', user_id, 'gradient_from_user', f_u, verbose=VERBOSE)
            assert str(response['ResponseMetadata']['HTTPStatusCode']) == '200' and f_u.shape == (n_users, K_VAL)
        except:
            return {'statusCode': response['ResponseMetadata']['HTTPStatusCode'], 'body': response['ResponseMetadata']}
    else:
        try:
            response = update_table_vector(gradient_y_table, 'user_id', user_id, 'gradient_from_user', f_u)
            assert str(response['ResponseMetadata']['HTTPStatusCode']) == '200' and f_u.shape == (n_users, K_VAL)
        except:
            return {'statusCode': response['ResponseMetadata']['HTTPStatusCode'], 'body': response['ResponseMetadata']}
    ...
    try:
        ...
        return {'statusCode': response['ResponseMetadata']['HTTPStatusCode'], 'body': 'Error getting f_u elements'}
    ...
    try:
        ...
        sort_user_id_raw = list(map(lambda user_id: get_mapped(demapping_table, 'user_id', str(user_id), field_user_id_raw), sort_index))
