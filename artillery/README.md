...
```js
function getMessageHandler(processor, context, params, ee, callback) {
  if (params.capture.handler !== undefined) {
    const done = () => callback(null, context)
    const error = (err) => callback(err, null)
    return (event) => (processor[params.capture.handler](event, error, done))
  } else {
      // ...
  }
...
WSEngine.prototype.step = function (requestSpec, ee) {
  ...
  const f = function (context, callback) {
    ...
    if (params.capture) {
      context.ws.onmessage = getMessageHandler(self.config.processor, context, params, ee, callback);
    }
  }
}
