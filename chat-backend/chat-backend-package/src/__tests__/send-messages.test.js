...
test('it sends messages', async () => {
  ...
  expect(sendMessageModule.sendMessage).toHaveBeenCalledWith({ user: { id: 'id-1' }, message: { action: 'action' } })
  expect(sendMessageModule.sendMessage).toHaveBeenCalledWith({ user: { id: 'id-2' }, message: { action: 'action' } })
})
